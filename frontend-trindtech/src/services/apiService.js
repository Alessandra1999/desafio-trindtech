...
export const getStudentCourseById = async (id_student, id_course) => {
  const response = await axios.get(
    `${API_URL}/student-course/${id_student}/${id_course}`
  );
  return response.data;
};
...
export const updateStudentCourse = async (
  ...
  try {
    ...
    console.error(
      "Erro ao atualizar associação aluno-curso:",
    ...
    for (const studentCourse of studentCourses) {
      if (studentCourse.id_student === id_student) {
        await deleteStudentCourse(
          studentCourse.id_student,
          studentCourse.id_course
        );
      }
    }
      ...
      studentCourses
        ...
        .map(async (sc) => {
          ...
          return {
            ...
            conclusionDate:
              conclusionDate || "Data de conclusão não encontrada!",
          };
        })
    );

    console.log(
      "Dados do aluno para atualizar: " + JSON.stringify(student, null, 2)
    );
    console.log(
      "Dados da localização para atualizar: " +
        JSON.stringify(location, null, 2)
    );
    console.log(
      "Dados do curso para atualizar: " +
        JSON.stringify(coursesWithConclusionDate, null, 2)
    );
...
export const checkStudentCourseAssociation = async (id_student, id_course) => {
  try {
    const response = await getStudentCourseById(id_student, id_course);
    ...
    if (response) {
      ...
      // Se os IDs do aluno e curso existirem na resposta, a associação existe
      if (studentId && courseId && conclusion_date) {
        console.log("Associação aluno-curso encontrada:", response);
        return true; // Associação encontrada
      } else {
        console.error(
          "Campos esperados não foram encontrados na resposta:",
          response
        );
      }
    } else {
      console.error("Resposta da API está vazia ou indefinida.", response);
    }

    return false; // Associação não encontrada
  } catch (error) {
    console.error("Erro ao verificar a associação aluno-curso:", error);
    return false;
  }
};
